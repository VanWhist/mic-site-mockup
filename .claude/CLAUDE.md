...
## 編集前に必ず行うこと
- 作業ブランチを切る前に `git fetch origin main` と `git status --short --branch` を
  実行し、作業ツリーが空であることを確認する。
- 未コミット変更がある場合は、pull・merge・rebase・resetを自動実行せず、状態を報告する。
- ブランチは必ず最新の origin/main から切る。切ったあとに自分のコミットが
  積み上がっている状態は正常であり、停止条件ではない。
- 編集のたびに対象ファイルを読み直してから編集する。
- 作業の途中で origin/main が進んだ場合でも、自分のブランチで作業を続けてよい。
  取り込みが必要かどうかはマージ時に判断する。
